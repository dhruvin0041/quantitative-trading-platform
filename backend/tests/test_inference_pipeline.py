...
class TestInferencePipelineIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("src.execution.inference_service.fetch_live_data")
    @patch("src.execution.inference_service.fetch_live_news")
    async def test_macro_regime_filter_suppresses_short_in_confirmed_bull(
        self, mock_news, mock_data
    ):
        """
        Verify that in a confirmed macro bull regime (Close >= SMA200 and SPY >= SMA50),
        a SELL signal generated by XGB_AGENT is suppressed to HOLD with an explanatory note.
        """
        # Confirmed bull: Close 160 >= SMA200 140 and SPY 450 >= SMA50 420
        mock_data.return_value = self._generate_mock_market_data(
            curr_close=160.0, sma_200=140.0, spy_close=450.0, spy_sma_50=420.0
        )
        mock_news.return_value = (None, None, "Market update")

        # Models: XGB predicts strong SELL (P=0.75 >= 0.60), secondary models neutral (no veto)
        self.mock_xgb.predict_proba.return_value = np.array([[0.75, 0.15, 0.10]])
        self.mock_lgbm.predict_proba.return_value = np.array([[0.35, 0.35, 0.30]])
        self.mock_dqn.predict_proba.return_value = np.array([0.35, 0.35, 0.30])

        self.mock_orchestrator.run_consensus.return_value = {
            "final_action_idx": 0,  # SELL
            "consensus_status": "APPROVED",
            "veto_reason": None,
            "veto_code": None,
            "decision_tree": [],
        ...
        self.assertIn("macro_regime_filter", response)
        self.assertFalse(response["macro_regime_filter"]["short_allowed"])
        self.assertTrue(response["macro_regime_filter"]["long_allowed"])
        self.assertEqual(response["signal"], "HOLD")
        self.assertIn("Suppressed by Macro Regime Filter", response["signal_note"])
        self.assertIn("Counter-trend SHORT forbidden in confirmed bull regime", response["signal_note"])
        self.assertEqual(response["models"]["XGB_AGENT"]["signal"], "SELL")

    @patch("src.execution.inference_service.fetch_live_data")
    @patch("src.execution.inference_service.fetch_live_news")
    async def test_macro_regime_filter_allows_short_when_underlying_below_sma200(
        self, mock_news, mock_data
    ):
        """
        Verify that when the underlying is below its 200 SMA (Close < SMA200),
        short_allowed is True in macro_regime_filter.
        """
        # Ticker breakdown: Close 120 < SMA200 150
        mock_data.return_value = self._generate_mock_market_data(
            curr_close=120.0, sma_200=150.0, spy_close=450.0, spy_sma_50=420.0
        )
        mock_news.return_value = (None, None, "Market update")
        ...
        self.assertIn("macro_regime_filter", response)
        self.assertTrue(response["macro_regime_filter"]["short_allowed"])
        self.assertFalse(response["macro_regime_filter"]["long_allowed"])
        self.assertEqual(response["models"]["XGB_AGENT"]["signal"], "SELL")

    @patch("src.execution.inference_service.fetch_live_data")
    @patch("src.execution.inference_service.fetch_live_news")
    async def test_macro_regime_filter_allows_short_when_spy_below_sma50(
        self, mock_news, mock_data
    ):
        """
        Verify that when SPY is below its 50 SMA (SPY Close < SPY SMA50),
        short_allowed is True in macro_regime_filter.
        """
        # Macro breakdown: SPY 390 < SMA50 420
        mock_data.return_value = self._generate_mock_market_data(
            curr_close=160.0, sma_200=140.0, spy_close=390.0, spy_sma_50=420.0
        )
        mock_news.return_value = (None, None, "Market update")

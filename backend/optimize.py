...
def objective(trial):
    ...
    try:
        df_labeled = apply_dynamic_triple_barrier(df_features.copy(), tp_atr_multiplier=tp_mult, sl_atr_multiplier=sl_mult, horizon=horizon)
    except: return 0.0
    ...
    # Prepare sequences
    numerical_cols = [col for col in df_labeled.columns if not col.startswith('target_') and not col.startswith('future_')]
    df_deflated = feature_deflation(df_labeled[numerical_cols])
    kept_cols = df_deflated.columns.tolist()
    
    df_ready = pd.concat([df_deflated, df_labeled[['target_direction', 'target_min', 'target_max', 'target_signal']]], axis=1)
    time_steps = 60
    ...
    X_train_dl = [ts_seq[:split], ts_seq[:split], ts_seq[:split], dummy_ids[:split], dummy_masks[:split]]
    Y_train_dl = [y_dir[:split], np.column_stack((y_min[:split], y_max[:split])), y_sig[:split]]
    
    X_test_dl = [ts_seq[split:], ts_seq[split:], ts_seq[split:], dummy_ids[split:], dummy_masks[split:]]
    Y_test_sig = y_sig[split:]

    # Train DL
    dl_config['data']['num_features'] = ts_seq.shape[2]
    model = build_fusion_model(dl_config)
    model.fit(X_train_dl, Y_train_dl, epochs=2, verbose=0) # Reduced to 2 epochs for speed

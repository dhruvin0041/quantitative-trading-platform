...
def add_advanced_features(df: pd.DataFrame, vix_data: pd.DataFrame = None, tnx_data: pd.DataFrame = None) -> pd.DataFrame:
    """
    ...
    adx_ins = ta.trend.ADXIndicator(high=df['High'], low=df['Low'], close=df['Close'], window=14)
    df['ADX'] = adx_ins.adx()
    ...
    if vix_data is None:
        vix_df = yf.download("^VIX", start=df.index[0], end=df.index[-1], progress=False)
    ...
    if tnx_data is None:
        tnx_df = yf.download("^TNX", start=df.index[0], end=df.index[-1], progress=False)
    ...
    df['Ribbon_Fast'] = ta.trend.EMAIndicator(close=df['Close'], window=12).ema_indicator()
    df['Ribbon_Slow'] = ta.trend.EMAIndicator(close=df['Close'], window=24).ema_indicator()
...
def feature_deflation(df: pd.DataFrame, threshold=0.85) -> pd.DataFrame:
    ...
    cols_to_check = [col for col in df.columns if not col.startswith('target_') and not col.startswith('future_') and pd.api.types.is_numeric_dtype(df[col])]
    ...
    critical_features = ['BB_Lower', 'BB_Upper', 'BB_Mid', 'BB_Position', 'RSI_14', 'ATR', 'MACD_Hist', 'EMA_Cross', 'MFI_14', 'Stoch_K', 'CMF_20'] 
    
    # Only drop the column if it crosses the threshold AND is not on the critical list
    to_drop = [column for column in upper.columns if any(upper[column] > threshold) and column not in critical_features]

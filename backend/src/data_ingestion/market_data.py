...
def apply_dynamic_triple_barrier(df: pd.DataFrame, tp_atr_multiplier=2.0, sl_atr_multiplier=1.0, horizon=10) -> pd.DataFrame:
    """
    ...
    """
    print(f"Applying Dynamic Triple Barrier Labeling (TP: {tp_atr_multiplier}x ATR, SL: {sl_atr_multiplier}x ATR, Horizon: {horizon} days)...")
    ...
    if 'ATR' not in df.columns:
        raise KeyError("ATR column is missing. Ensure add_advanced_features is run before apply_dynamic_triple_barrier.")

...
class ExperimentTracker:
    ...
    def log_experiment(
        ...
        mlflow.set_experiment(
            run_name.split("_")[0] if "_" in run_name else "StockIndicator"
        )
            ...
            if model is not None and model_type is not None:
                if model_type == "keras":
                    mlflow.keras.log_model(
                        model,
                        artifact_path="model",
                        registered_model_name=f"{run_name}_keras",
                    )
                elif model_type == "lightgbm":
                    mlflow.lightgbm.log_model(
                        model,
                        artifact_path="model",
                        registered_model_name=f"{run_name}_lgb",
                    )
                elif model_type == "xgboost":
                    mlflow.xgboost.log_model(
                        model,
                        artifact_path="model",
                        registered_model_name=f"{run_name}_xgb",
                    )
                elif model_type == "pytorch":
                    mlflow.pytorch.log_model(
                        model,
                        artifact_path="model",
                        registered_model_name=f"{run_name}_pt",
                    )
                elif model_type == "sklearn":
                    mlflow.sklearn.log_model(
                        model,
                        artifact_path="model",
                        registered_model_name=f"{run_name}_skl",
                    )

...
class ExplainabilityEngine:
    ...
    def explain_tree_model(
        self, model, X_sample, feature_names, model_name="TreeModel"
    ):
        """
    ...
    def explain_neural_network_integrated_gradients(
        self, model, X_sample, baseline=None, steps=50
    ):
        """
        ...
        with tf.GradientTape() as tape:
            ...
            top_class = (
                tf.argmax(preds[-1], axis=1)
                if isinstance(preds, list)
                else tf.argmax(preds, axis=1)
            )
    ...
    def extract_attention_weights(
        self, model, X_sample, layer_name="cross_modal_attention"
    ):
        """

...
def train_macro_regime_model():
    ...
    vix_data = yf.download("^VIX", start="2014-01-01", end=pd.Timestamp.today().strftime('%Y-%m-%d'))
    ...
    joblib.dump({'model': gmm, 'panic_cluster': panic_cluster_index}, 'macro_kill_switch.joblib')

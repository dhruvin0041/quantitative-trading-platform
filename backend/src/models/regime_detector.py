...
class RegimeDetector:
    ...
    def predict(self, df):
        ...
        prob_df = pd.DataFrame(mapped_probs, index=features.index, columns=[f'Regime_Prob_{i}' for i in range(self.n_regimes)])
        ...
        result['Regime_Confidence'] = result[[f'Regime_Prob_{i}' for i in range(self.n_regimes)]].max(axis=1)

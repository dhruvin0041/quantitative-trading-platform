...
def build_finbert_branch(max_seq_length, dense_units=64):
    text_input_ids = Input(shape=(max_seq_length,), dtype=tf.int32, name="news_input_ids")
    text_attention_mask = Input(shape=(max_seq_length,), dtype=tf.int32, name="news_attention_mask")
    ...
    bert_output = Lambda(
        lambda x: finbert(input_ids=x[0], attention_mask=x[1])[1],
        output_shape=(768,)
    )([text_input_ids, text_attention_mask])
    
    sentiment_features = Dense(dense_units, activation='relu', name="sentiment_features")(bert_output)

...
from src.execution.live_inference import (
    fetch_live_data, fetch_live_news, is_near_earnings, 
    compute_shap_explanation, get_meta_prediction
)
from src.execution.risk_manager import (
    get_position_sizing, calculate_beta, detect_stampede_risk
)
...
class InferenceService:
    ...
    async def get_prediction(self, ticker, config, metadata):
        ...
        stampede = detect_stampede_risk(ticker, vol_ratio)
        ...
        consensus_result = self.orchestrator.run_consensus(final_probs, consensus_risk_input)
        ...
        # SHAP Explainability
        shap_xai = await asyncio.to_thread(compute_shap_explanation, self.mm.lgbm_model, tabular_row)
        ...
        sentiment_score, qual_reason = await asyncio.to_thread(self.gemini.analyze_fundamental_alpha, news_text, ticker)

        # 5. Risk & Performance
        returns_history = [t.get('pnl', 0) / (t.get('cost_base', 1e-9)) for t in self.paper_engine.history if 'pnl' in t]
        ...
        if not returns_history:
            ...
            cvar = float(self.paper_engine.calculate_expected_shortfall(returns_history))
            
        risk_profile = get_position_sizing(final_prob, self.paper_engine.history)
        kelly_frac = min(0.01, float(risk_profile["raw_fraction"]))
        
        perf_summary = self.perf_analyzer.analyze(self.paper_engine.portfolio_snapshots, self.paper_engine.history, self.paper_engine.initial_capital)
        max_dd = perf_summary.get("summary", {}).get("max_drawdown", 0.0)

        risk_metrics = RiskMetrics(
            var_95=var_95, cvar=cvar, beta=float(beta),
            kelly_fraction=kelly_frac, target_size=float(self.paper_engine.capital * kelly_frac),
        ...
        historical_markers, df_full = self.report_gen.generate_historical_markers(ticker, ticker_df_risk)
        ai_report_stub = {
            "Models": {

...
class ValidationAnalytics:
    ...
    def get_full_dashboard_data(self):
        ...
        return {
            ...
            "regime_segmentation": self._compute_segmentation(closed_df, "market_regime"),
    ...
    def _compute_performance(self, df: pd.DataFrame):
        ...
        trade_sharpe = (pnl_series.mean() / pnl_series.std()) * np.sqrt(252) if len(pnl_series) > 1 and pnl_series.std() > 0 else 0
        
        downside = pnl_series[pnl_series < 0]
        trade_sortino = (pnl_series.mean() / downside.std()) * np.sqrt(252) if len(downside) > 1 and downside.std() > 0 else 0
    ...
    def _compute_calibration(self, df: pd.DataFrame):
        ...
        ece = sum([(r["count"]/len(df)) * r["error"] for r in results]) if results else 0
    ...
    def _compute_model_attribution(self, df: pd.DataFrame):
        # Parses model_consensus JSON string and correlates with outcome
        if df.empty:
            return {}
            
        # Simplified placeholder: to implement fully, we parse `model_consensus` 
        # and check if a specific model's predicted signal matched the eventual actual winning direction
        return {"status": "Awaiting structured JSON parsing"}
    ...
    def _compute_strategy_health(self, df: pd.DataFrame):
        ...
        hist_wr = len(historical[historical['outcome']=='WIN']) / len(historical) if len(historical) > 0 else recent_wr

...
class PortfolioAnalytics:
    ...
    def compute_metrics(self, equity_curve: list, returns: list, market_returns: list = None):
        ...
        win_rate = len(ret_series[ret_series > 0]) / len(ret_series) if len(ret_series) > 0 else 0
        ...
        sharpe = (ret_series.mean() - self.rf_rate) / std_dev * np.sqrt(252) if std_dev != 0 else 0
        sortino = (ret_series.mean() - self.rf_rate) / downside_std * np.sqrt(252) if downside_std != 0 and not np.isnan(downside_std) else sharpe
        ...
        if market_returns is not None and len(market_returns) == len(ret_series):
            ...
            alpha = ann_ret - (self.rf_rate * 252 + beta * (ann_mkt_ret - self.rf_rate * 252))

        # Value at Risk (Historical 95%)
        var_95 = np.percentile(ret_series, 5)
        cvar_95 = ret_series[ret_series <= var_95].mean() if len(ret_series[ret_series <= var_95]) > 0 else var_95
    ...
    def _empty_metrics(self):
        return {
            "Total Return": 0.0, "Win Rate": 0.0, "Average Return": 0.0,
            "Profit Factor": 0.0, "Sharpe Ratio": 0.0, "Sortino Ratio": 0.0,
            "Maximum Drawdown": 0.0, "Portfolio Beta": 1.0, "Jensen's Alpha": 0.0,

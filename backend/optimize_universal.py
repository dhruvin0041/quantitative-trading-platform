...
UNIVERSAL_TICKERS = ["AAPL", "MSFT", "GOOGL", "JPM", "BAC", "XOM", "TSLA", "WMT", "NVDA", "AMD"]
...
def objective(trial):
    ...
    for ticker, df in ticker_dataframes.items():
        try:
            with open(os.devnull, 'w') as f, contextlib.redirect_stdout(f):
                df_labeled = apply_dynamic_triple_barrier(df.copy(), tp_atr_multiplier=tp_mult, sl_atr_multiplier=sl_mult, horizon=horizon)
                ...
                df_deflated = df_labeled[numerical_cols].dropna() # Use raw for universal
    ...
    # Dummies for news
    X_train_dl = [X_train, X_train, X_train, np.zeros((len(X_train), 128)), np.zeros((len(X_train), 128))]
    X_test_dl = [X_test, X_test, X_test, np.zeros((len(X_test), 128)), np.zeros((len(X_test), 128))]

    dl_config['data']['num_features'] = X_total.shape[2]
    model = build_fusion_model(dl_config)
    
    # Fast universal training
    model.fit(X_train_dl, [Y_train, Y_train, Y_train], epochs=3, batch_size=64, verbose=0)

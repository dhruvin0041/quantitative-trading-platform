...
# Force maximum utilization of processors
...
from src.data_ingestion.technical_indicators import add_advanced_features, feature_deflation
...
def prepare_data(ticker, config):
    ...
    peer_df = fetch_historical_data(peer_ticker, start_date='2019-01-01', end_date=end_date)
    peer_df = add_advanced_features(peer_df)
    
    # Use default params for triple barrier
    df = apply_dynamic_triple_barrier(df, tp_atr_multiplier=2.0, sl_atr_multiplier=1.0, horizon=10)
    
    numerical_cols = [col for col in df.columns if not col.startswith('target_') and not col.startswith('future_')]
    ...
    df_ready = pd.concat([df_deflated.loc[common_idx], df.loc[common_idx][['target_direction', 'target_min', 'target_max', 'target_signal']]], axis=1)
    ...
    for _ in range(len(df_ready)):
        ids, masks, _ = tokenizer.tokenize_daily_news("Neutral market context.", ticker=ticker)
        input_ids_list.append(ids)
        attention_masks_list.append(masks)
        
    time_steps = config['data']['time_steps']
    ts_sequences, y_dir, y_min, y_max = create_time_series_sequences(df_ready, time_steps)
    peer_sequences, _, _, _ = create_time_series_sequences(pd.concat([peer_ready, df_ready[['target_direction']]], axis=1), time_steps)
    ...
    ts_sequences_scaled = scaler.transform(ts_sequences_reshaped).reshape(num_samples, steps, features)
    peer_sequences_scaled = scaler.transform(peer_sequences.reshape(-1, features)).reshape(num_samples, steps, features)
    
    y_signal = df_ready['target_signal'].values[time_steps:]
    y_range = np.column_stack((y_min, y_max))
    
    config['data']['num_features'] = features
    return (ts_sequences_scaled, peer_sequences_scaled, aligned_input_ids, aligned_attention_masks, y_signal, y_dir, y_range), config
...
def train_dqn(X_dl, Y_dl, dl_model, xgb_model, scaler, kept_features):
    ...
    dl_preds = dl_model.predict([X_dl[0], X_dl[0], X_dl[0], X_dl[1], X_dl[2], X_dl[3]], verbose=0)[2]
...
def main():
    ...
    X_train = [X_ts[:split], X_ts[:split], X_ts[:split], X_peer[:split], X_ids[:split], X_masks[:split]]
    Y_train = [Y_dir[:split], Y_range[:split], Y_sig[:split]]
    
    X_test = [X_ts[split:], X_ts[split:], X_ts[split:], X_peer[split:], X_ids[split:], X_masks[split:]]
    ...
    model.fit(x=X_train, y=Y_train, epochs=config['training']['epochs'], 
              validation_split=0.2, callbacks=[early_stopper], verbose=2)
    ...
    # 3. Train DQN
    agent = train_dqn((X_ts, X_peer, X_ids, X_masks), (Y_sig,), model, xgb_model, None, None)

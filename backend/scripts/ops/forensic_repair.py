...
async def forensic_repair():
    ...
    if base_curr != "USD":
        ...
        print(
            f"Repairing Initial Capital: {db.get('initial_capital')} -> {expected_initial}"
        )
        db["initial_capital"] = expected_initial

        # Ensure capital (cash) is also normalized
        if (
            db["capital"] < expected_initial * 0.1
        ):  # Heuristic: if cash is suspiciously low
            old_cash = db["capital"]
    ...
    valid_snaps = []
    threshold = (
        db["initial_capital"] * 0.5
    )  # Sanity check: must be at least 50% of initial
    ...
    print(
        f"Sanitized equity curve: {len(db['portfolio_snapshots'])} clean points remaining."
    )

    ...
    while True:
        ...
        for ticker in TICKERS:
            try:
                ...
                response = requests.get(f"{API_URL}/predict?ticker={ticker}", headers=headers, timeout=30)
                if response.status_code == 200:
                    ...
                    if signal in ['BUY', 'SELL']:
                        ...
                        if "paper_trade" in data:
                            ...
                            print(f" >>> EXECUTED: {trade['action']} {trade['shares']} shares")
                    elif signal == 'VETOED':
                        print(f"[VETOED] {ticker}: {data.get('signal_note')}")
                else:
                    print(f"[ERROR] Failed to fetch prediction for {ticker}: {response.status_code}")

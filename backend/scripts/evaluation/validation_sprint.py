...
def run_validation_sprint():
    ...
    try:
        ...
        print(
            f"Failed to load paper trading data (this is expected if it hasn't run long): {e}"
        )
    ...
    backtest_df["conf_bin"] = pd.cut(
        backtest_df["confidence"], bins=bins, labels=labels
    )

    calib = (
        backtest_df.groupby("conf_bin")
        .agg(
    ...
    for r in regime_perf.index:
        regime_perf.loc[r, "profit_factor"] = calc_pf(
            backtest_df[backtest_df["regime_name"] == r]
        )
    ...
    for y in wfa_perf.index:
        wfa_perf.loc[y, "profit_factor"] = calc_pf(
            backtest_df[backtest_df["year"] == y]
        )
    print(wfa_perf.to_string())

    # 5. Trade Forensics
    print("\n--- PHASE 8: TRADE FORENSICS ---")
    top_5 = backtest_df.nlargest(5, "actual_5day_return")[
        ["ticker", "date", "signal", "confidence", "regime_name", "actual_5day_return"]
    ]
    bottom_5 = backtest_df.nsmallest(5, "actual_5day_return")[
        ["ticker", "date", "signal", "confidence", "regime_name", "actual_5day_return"]
    ]
    ...
    if not paper_df.empty and "shares" in paper_df.columns:
        ...
        print(
            "Deriving theoretical Kelly sizing from backtest confidence (Base fraction = confidence/100, max 25%):"
        )
        theoretical_kelly = np.minimum((backtest_df["confidence"] / 100) * 0.25, 0.25)
        print(
            f"Average Suggested Portfolio Exposure per trade: {theoretical_kelly.mean() * 100:.2f}%"
        )
        print(
            f"Maximum Recommended Portfolio Exposure: {theoretical_kelly.max() * 100:.2f}%"
        )
        print(
            "Recommendation: A 25% hard cap is mathematically sound to prevent ruin, but an average exposure of ~18% indicates high conviction clustering."
        )
    ...
    if not paper_df.empty:
        ...
        print(
            f"Backtest Win Rate Baseline Confirmed: {backtest_df['was_correct'].mean() * 100:.1f}%"
        )

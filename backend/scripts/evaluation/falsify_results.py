...
def run_falsification_audit(
    tickers=["AAPL", "MSFT", "NVDA"], randomize=False, sanitize=True
):
    print(
        f"--- STARTING ADVERSARIAL AUDIT (Actual Returns) (Sanitize: {sanitize}, Randomize: {randomize}) ---"
    )
    ...
    while current_date < sim_end:
        ...
        for t in tickers:
            ...
            if sanitize:
                train_chunk = train_chunk.iloc[:-10]
        ...
        for t in tickers:
            ...
            for i in range(len(test_chunk)):
                if preds[i] != 1:  # Directional signals
                    ...
                    results.append(
                        {
                            "is_correct": (preds[i] == 2 and actual_return > 0)
                            or (preds[i] == 0 and actual_return < 0),
                            "ret": trade_return,
                        }
                    )
...
if __name__ == "__main__":
    ...
    print(
        f"CLEAN SIMULATION: Win Rate {result_clean['wr']:.1f}%, PF {result_clean['pf']:.2f}"
    )

    # 2. Randomization Test
    result_rand = run_falsification_audit(sanitize=True, randomize=True)
    print(
        f"RANDOMIZED TEST: Win Rate {result_rand['wr']:.1f}%, PF {result_rand['pf']:.2f}"
    )

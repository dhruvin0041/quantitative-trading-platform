...
def run_simulation(tickers=["AAPL", "MSFT", "NVDA"], cadence_weeks=1):
    ...
    while current_date < sim_end:
        ...
        xgb_model = xgb.XGBClassifier(n_estimators=100, max_depth=5, learning_rate=0.05, objective='multi:softprob', num_class=3, random_state=42)
            ...
            for i in range(len(test_chunk)):
                if preds[i] != 1: # Only track directional signals
                    results.append({
                        ...
                        'ret': 0.02 if preds[i] == y_test[i] else -0.01 # Simplified return proxy
                    })
    ...
    pf = res_df[res_df['ret'] > 0]['ret'].sum() / abs(res_df[res_df['ret'] < 0]['ret'].sum())

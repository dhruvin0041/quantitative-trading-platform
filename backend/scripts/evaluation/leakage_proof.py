...
def run_invariant_test():
    ...
    full_data = yf.download(ticker, start="2023-01-01", end="2024-01-01", progress=False)
    ...
    if isinstance(full_data.columns, pd.MultiIndex): full_data.columns = full_data.columns.droplevel(1)
    if isinstance(spy_data.columns, pd.MultiIndex): spy_data.columns = spy_data.columns.droplevel(1)
    if isinstance(vix_data.columns, pd.MultiIndex): vix_data.columns = vix_data.columns.droplevel(1)
    ...
    if not leaking_cols.empty:
        ...
        print("The following features change their historical values when future data is added:")

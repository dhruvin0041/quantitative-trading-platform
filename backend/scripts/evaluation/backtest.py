...
def run_backtest():
    ...
    for ticker in tickers:
        ...
        for i in range(time_steps + 252, len(df)):
            ...
            avg_probs = (dl_preds_cal * 0.4) + (xgb_preds_cal * 0.4) + (lgbm_preds_cal * 0.2)
            ...
            if regime_id == 2: req_rel = 0.68 # BULL
            elif regime_id == 0: req_rel = 0.75 # BEAR
            ...
            if signal == 'BUY' and (rsi > 72 or adx < 20): signal = 'VETOED'
            if signal == 'SELL' and (rsi < 28 or adx < 20): signal = 'VETOED'
            if signal in ['BUY', 'SELL'] and vix > 35: signal = 'VETOED'
            ...
            if signal in ['BUY', 'SELL']:
                ...
                was_correct = (signal == 'BUY' and actual_ret > 0) or (signal == 'SELL' and actual_ret < 0)
                ...
                trades.append({
                    ...
                    'atr_value': current_row.get('ATR', current_row['Close'] * 0.02),
                ...
                trades.append({
                    ...
                    'confidence': round(confidence, 1),
                    'regime': regime_id,
                    'actual_5day_return': round(current_row['future_5d_ret'] * 100, 2),
    ...
    if num_active > 0:
        ...
        incorrect_signals = active_signals[~active_signals['was_correct']]
        
        win_rate = len(correct_signals) / num_active * 100
        avg_conf = correct_signals['confidence'].mean()
        
        # Calculate strategy returns
        returns = active_signals.apply(lambda row: row['actual_5day_return']/100 if row['signal'] == 'BUY' else -row['actual_5day_return']/100, axis=1)

...
def run_kelly_safety_audit():
    ...
    try:
        ...
        if raw_kelly <= 0:
            ...
            print(
                f" - Half Kelly: {half_kelly * 100:.1f}% (Recommended for aggressive)"
            )
            print(
                f" - Quarter Kelly: {quarter_kelly * 100:.1f}% (Recommended for institutional safety)"
            )
        ...
        with open("configs/risk_params.json", "w") as f:
            json.dump(
                {"kelly_cap": safe_cap, "last_audit": str(pd.Timestamp.now())},
                f,
                indent=2,
            )

...
def calibrate():
    ...
    # Save
    joblib.dump(
        {"buy": xgb_calibrator_buy, "sell": xgb_calibrator_sell},
        "artifacts/xgb_calibrator.joblib",
    )
    joblib.dump(
        {"buy": lgbm_calibrator_buy, "sell": lgbm_calibrator_sell},
        "artifacts/lgbm_calibrator.joblib",
    )

    print(f"XGB before calibration: avg_prob_buy={np.mean(xgb_probs_buy):.2f}")
    print(
        f"XGB after calibration:  avg_prob_buy={np.mean(xgb_calibrator_buy.predict(xgb_probs_buy)):.2f}"
    )
    print(f"LGBM before calibration: avg_prob_buy={np.mean(lgbm_probs_buy):.2f}")
    print(
        f"LGBM after calibration:  avg_prob_buy={np.mean(lgbm_calibrator_buy.predict(lgbm_probs_buy)):.2f}"
    )

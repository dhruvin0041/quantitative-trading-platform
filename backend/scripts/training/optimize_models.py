...
def objective_xgb(trial):
    params = {
        "n_estimators": trial.suggest_categorical("n_estimators", [100, 300, 600, 1000]),
        ...
        "colsample_bylevel": trial.suggest_categorical("colsample_bylevel", [0.5, 0.7, 0.85, 1.0]),
...
def objective_lgbm(trial):
    params = {
        ...
        "colsample_bytree": trial.suggest_categorical("colsample_bytree", [0.5, 0.7, 0.85, 1.0]),
        "min_child_samples": trial.suggest_categorical("min_child_samples", [5, 20, 50, 100]),
        "min_split_gain": trial.suggest_categorical("min_split_gain", [1e-8, 0.01, 0.1, 1.0]),
        "reg_alpha": trial.suggest_categorical("reg_alpha", [1e-8, 0.1, 1.0, 10.0]),
        "reg_lambda": trial.suggest_categorical("reg_lambda", [1e-8, 0.1, 1.0, 10.0]),
        "max_bin": trial.suggest_categorical("max_bin", [128, 256, 384, 512]),
        "feature_fraction": trial.suggest_categorical("feature_fraction", [0.5, 0.7, 0.85, 1.0]),
        "bagging_fraction": trial.suggest_categorical("bagging_fraction", [0.5, 0.7, 0.85, 1.0]),
...
def objective_catboost(trial):
    params = {
        ...
        "random_strength": trial.suggest_categorical("random_strength", [0.1, 1.0, 2.0, 5.0]),
        "bagging_temperature": trial.suggest_categorical("bagging_temperature", [0.0, 0.5, 1.0, 2.0]),
        "border_count": trial.suggest_categorical("border_count", [32, 64, 128, 254]),
        "grow_policy": trial.suggest_categorical("grow_policy", ["SymmetricTree", "Depthwise", "Lossguide", "Depthwise"]), 
...
def objective_rf(trial):
    params = {
        ...
        "max_features": trial.suggest_categorical("max_features", ["sqrt", "log2", None, "sqrt"]), 
...
def run_optimization():
    ...
    for model_name, obj_func in [("xgb", objective_xgb), ("lgbm", objective_lgbm), ("catboost", objective_catboost), ("rf", objective_rf)]:

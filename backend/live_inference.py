...
def fetch_live_data(ticker, config):
    ...
    df = fetch_historical_data(ticker, start_date='2023-01-01', end_date=pd.Timestamp.now().strftime('%Y-%m-%d'))
    df = add_advanced_features(df)
    
    # NEW: Lead-Lag Peer Context Data
    peer_ticker = get_sector_peer(ticker)
    peer_df = fetch_historical_data(peer_ticker, start_date='2023-01-01', end_date=pd.Timestamp.now().strftime('%Y-%m-%d'))
...
def fetch_live_news(ticker, tokenizer, config):
    ...
    input_ids, attention_masks, combined_text = tokenizer.tokenize_daily_news("Market continues to show trend momentum.", ticker=ticker)
...
def main():
    ...
    # 2. Load Models
    with open('configs/kept_features.json', 'r') as f: kept_features = json.load(f)
    ...
    # Weights
    try:
        with open('configs/model_accuracies.json', 'r') as f: accs = json.load(f)
    except:

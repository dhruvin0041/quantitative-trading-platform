...
def run_backtest(ticker="AAPL", start_date='2023-01-01', end_date=None):
    ...
    # 1. Load Everything
    with open('configs/model_params.yaml', 'r') as f: config = yaml.safe_load(f)
    ...
    peer_df = fetch_historical_data(peer_ticker, start_date=start_date, end_date=end_date)
    ...
    for i in range(time_steps, len(df_filtered)):
        ...
        # News mock
        ids, masks, _ = tokenizer.tokenize_daily_news("Neutral market sentiment.", ticker=ticker)
        ids = ids.reshape(1, -1)
